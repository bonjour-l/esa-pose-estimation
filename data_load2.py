...
class LinemodDataSet(Dataset):
    def __init__(self, root, name='cat', train=True, use_fuse=True, use_render=True, scale=128):
        ...
        if (self.train == True):
            ...
            with open(real_filename, 'rb') as fo:  # 读取pkl文件数据
                self.real_data = pickle.load(fo, encoding='bytes')
                self.real_test_data = []
                test_filename = "data/" + name + "_test.pkl"
    ...
    def __getitem__(self, item):
        ...
        for point in des['sift']:
            heatmap = CenterLabelHeatMap(left, down, rate * (point[0] - x_new), rate * (point[1] - y_new), 2)
        ...
        return [image, mask, label, weights], [bbox, rate, des['sift'], des['sift_3d'], des['K'],
...
if __name__ == "__main__":
    test_data = LinemodDataSet('/home/lin/Documents/6D/pvnet/pvnet-master/data/LINEMOD/')
    [c_data, data] = test_data.__getitem__(0)
