...
class ESADataSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        des = self.data[item]
        # print(des['K'])
        #print(des['sift3d'].shape)
        img_name=des['rgb_pth']
        #print(len(img_name))
        if len(img_name)==13:
            img = read_mask_np(self.root +'train/'+ des['rgb_pth'])
        else:
            img = read_mask_np(self.root +'real_test/'+ des['rgb_pth'])
...
if __name__ == "__main__":
    ...
    [c_data, data] = train_data.__getitem__(64)
    image,  label, weights = c_data
    bbox, rate, sift, sift_3d, K,RT,qua,_=data
