...
def pose_nms(bboxes, pose_preds, pose_scores):
    ...
    pose_preds:     pose locations list (n, nClasses, 2)
    pose_scores:    pose scores list    (n, nClasses, 1)
    '''
    ...
    while(human_scores.shape[0] != 0):
        ...
        pose_preds = np.delete(pose_preds.cpu(), delete_ids, axis=0)
        pose_scores = np.delete(pose_scores.cpu().detach().numpy(), delete_ids, axis=0)
        human_ids = np.delete(human_ids, delete_ids)
        human_scores = np.delete(human_scores.cpu().detach().numpy(), delete_ids, axis=0)
    ...
    for j in range(len(pick)):
        # modified by @pengggao, variable joints
        ids = np.arange(32)
...
def p_merge(ref_pose, cluster_preds, cluster_scores, ref_dist):
    ...
    INPUT:
        ref_pose:       reference pose          -- [nClasses, 2]
        cluster_preds:  redundant poses         -- [n, nClasses, 2]
        cluster_scores: redundant poses score   -- [n, nClasses, 1]
        ref_dist:       reference scale         -- Constant
    OUTPUT:
        final_pose:     merged pose             -- [nClasses, 2]
        final_score:    merged score            -- [nClasses]
    '''
...
def get_parametric_distance(i, all_preds, keypoint_scores, ref_dist):
    ...
    # modified by @pengggao, variable joints
    score_dists = torch.zeros(all_preds.shape[0],32).cuda()
